...
DayOneExport.prototype.createText = function createText(day) {
    ...
    day.segments.forEach(function(segment) {
        ...
        if(segment.type == 'place') {
            ...
            var cd = countdown(start, end, countdown.DAYS | countdown.HOURS | countdown.MINUTES, 2);
            if(!placeTime.has(placeName))
                placeTime.set(placeName, cd);
            else
            {
                var days = placeTime.get(placeName).days + cd.days;
                var hours = placeTime.get(placeName).hours + cd.hours;
                var minutes = placeTime.get(placeName).minutes + cd.minutes;
                var duration = countdown(moment(Date.now()).subtract(days, 'day').subtract(hours, 'hour').subtract(minutes, 'minute'), Date.now(), countdown.ALL, 2);
                placeTime.set(placeName, duration);
            }
    ...
    var pl = []
    placeTime.forEach(function (value, key) {
        pl.push({'name': key, 'duration': value});
    });

    pl.sort(function(a, b){return b['duration'].value - a['duration'].value})

    pls = []
    pl.forEach(function(place, index){
        pls.push(place['duration'].toString() + ' at ' + place['name']);
    })
